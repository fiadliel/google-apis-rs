...
use google_cloudiot1::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _projects_locations_registries_devices_config_versions_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "num-versions" => {
                    call = call.num_versions(        value.map(|v| arg_from_str(v, err, "num-versions", "int32")).unwrap_or(-0));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
    ...
    async fn _projects_locations_registries_devices_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "device-num-ids" => {
                    call = call.add_device_num_ids(        value.map(|v| arg_from_str(v, err, "device-num-ids", "uint64")).unwrap_or(0));
                },
                "device-ids" => {
    ...
    async fn _projects_locations_registries_devices_states_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "num-states" => {
                    call = call.num_states(        value.map(|v| arg_from_str(v, err, "num-states", "int32")).unwrap_or(-0));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
