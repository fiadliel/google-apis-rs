...
use google_runtimeconfig1_beta1::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _projects_configs_variables_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "return-values" => {
                    call = call.return_values(        value.map(|v| arg_from_str(v, err, "return-values", "boolean")).unwrap_or(false));
                },
                "page-token" => {
                    call = call.page_token(value.unwrap_or(""));
                },
