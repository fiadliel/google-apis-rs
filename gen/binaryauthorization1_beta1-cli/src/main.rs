...
use google_binaryauthorization1_beta1::{api, Error, oauth2, client::chrono, FieldMask};
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["delegation-service-account-email", "description", "etag", "name", "note-reference", "update-time", "user-owned-drydock-note"]);
