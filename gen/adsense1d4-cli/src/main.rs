...
use google_adsense1d4::{api, Error, oauth2, client::chrono, FieldMask};
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _accounts_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "tree" => {
                    call = call.tree(        value.map(|v| arg_from_str(v, err, "tree", "boolean")).unwrap_or(false));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "use-timezone-reporting" => {
                    call = call.use_timezone_reporting(        value.map(|v| arg_from_str(v, err, "use-timezone-reporting", "boolean")).unwrap_or(false));
                },
                "start-index" => {
