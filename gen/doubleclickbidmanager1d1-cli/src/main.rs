...
use google_doubleclickbidmanager1d1::{api, Error, oauth2, client::chrono, FieldMask};
...
struct Engine<'n, S> {
    opt: ArgMatches<'n>,
    hub: api::DoubleClickBidManager<S>,
    gp: Vec<&'static str>,
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    async fn _queries_createquery(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "asynchronous" => {
                    call = call.asynchronous(        value.map(|v| arg_from_str(v, err, "asynchronous", "boolean")).unwrap_or(false));
                },
                _ => {
                    let mut found = false;
